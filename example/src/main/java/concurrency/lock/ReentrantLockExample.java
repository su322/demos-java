 ...
 * 3. 可中断锁获取 见 ReentrantLockFeaturesExample
 * 支持 lock.lockInterruptibly()，允许线程在等待锁时响应中断，避免死等。
 * 4. 尝试获取锁 见 ReentrantLockFeaturesExample
 * 支持 lock.tryLock()，可以立即返回是否获取到锁，或指定超时时间，适合需要避免长时间阻塞的场景。
 * 5. 公平锁/非公平锁 见 ReentrantLockFeaturesExample
 * 构造时可指定公平性（new ReentrantLock(true)），公平锁保证线程按请求顺序获得锁，非公平锁则可能插队，性能更高。
 * 6. 支持条件变量 见 ProducerConsumerWithReentrantLockAndConditionExample
